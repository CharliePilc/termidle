...
class CLIPlayer:
    ...
    def handle_response(self, guess: str, states: List[LetterStates], hint: int):
        ...
        self.out(
            self.pretty_response(guess, states, self._C)
            + (f" { self._C.DIM }{ hint } possible" if hint != -1 else "")
        )
        self.update_keyboard()
    ...
    def update_keyboard(self):
        if self._lines_since_keyboard >= 1:
            sys.stdout.write(
                f"\033[{self._lines_since_keyboard}F"
            )  # move cursor up to keyboard line
        sys.stdout.write(
            self.pretty_response(
                list(self._keyboard_status.keys()),
                list(self._keyboard_status.values()),
                self._C,
            )
            + "\xA0"
        )
        if self._lines_since_keyboard >= 1:
            sys.stdout.write(
                f"\033[{self._lines_since_keyboard}E"
